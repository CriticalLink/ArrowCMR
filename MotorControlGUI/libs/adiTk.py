...
class AdiWindow(tk.Tk):
    ...
    def __init__(self, name, close_cb):
        ...
        self.title(name + ' ' + chr(169) + ' Analog Devices 2020')
...
class AdiFrame(tk.Frame):
    ...
    def __init__(self, parent, name, row=0, column=0, rowspan=1, columnspan=1,
...
class AdiCanvasFrame(AdiFrame):
    ...
    def __init__(self, parent, name="plotting frame", row=0, column=0, rowspan=2, columnspan=2):
        AdiFrame.__init__(
            self,
            parent,
            name,
        ...
        self.parent = parent
        self.f = fig.Figure(dpi=100)  # matplotlib figure to embed in tkinter
...
class AdiOptionsFrame(AdiFrame):
    ...
    def __init__(self, parent, motor, toggle_plot, log_data, send_down_samp,
                 show_raw_data, toggle_fft, _hold_data, _clear_data,
        ...
        check_options = {
            ...
            "Raw ADC Data": (self._show_raw_data, self.raw_data,
                             "Plot raw data from the SINC filter"),
    ...
    def _show_raw_data(self):
        if self.raw_data.get():  # check button ticked
            self.show_raw_data(True)
            print("Showing raw data")
        else:  # check button not ticked
            self.show_raw_data(False)
            print("Showing float formatted data")
